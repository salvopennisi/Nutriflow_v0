...
**Nutriflow** è un'applicazione web in Python/Streamlit per supportare il lavoro operativo di un nutrizionista: gestione degli assistiti, stima e aggiornamento del fabbisogno energetico, rilevazioni biometriche, analisi della composizione corporea e costruzione di piani alimentari settimanali con calcolo di macro e micronutrienti.
...
- salvare le risposte di un assistito sostituendo atomically il set precedente.
...
## Epic D - Catalogo e configurazioni backend
...
La copertura non include ancora `diet_service`, `food_service` e i flussi UI Streamlit/AG Grid.
...
2. correggere naming e trend del modulo biometrico;
3. rimuovere il `user_id` placeholder introducendo un'identità applicativa coerente;
4. aggiornare devcontainer e script di bootstrap;
5. aggiungere test per `diet_service` e micronutrienti;
6. esporre, se necessario, una UI dedicata per catalogo alimenti e riferimenti micronutrienti;
7. collegare alla UI il questionario di anamnesi strutturato già supportato dal backend.
...
**Nutriflow** è attualmente focalizzato su tre flussi operativi principali: **assistito → biometria → piano alimentare**, con un editor dietetico già ricco di logiche di calcolo e una base backend pronta per ulteriori funzionalità di catalogazione e anamnesi strutturata.
